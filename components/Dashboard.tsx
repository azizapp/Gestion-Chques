...
import { predictCashFlow } from '../services/geminiService.ts';
...
const Dashboard: React.FC<DashboardProps> = ({ checks, currency, onTabChange, isAdmin }) => {
  ...
  const fetchCashFlow = async () => {
    ...
    const result = await predictCashFlow(checks, currency);
    setCashFlow(result);
    ...
    <div className="space-y-8 animate-in fade-in duration-1000">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-8 rounded-[22px] border-emerald-500/10 bg-emerald-500/[0.02] shadow-2xl relative overflow-hidden">
            <div className="flex items-center justify-between mb-4">
              <h5 className="text-[10px] font-black text-emerald-400 uppercase tracking-[0.2em] flex items-center gap-2">
                <LineChart size={14} /> Prévision de Trésorerie
              </h5>
              {loadingCashFlow && <Loader2 size={14} className="animate-spin text-emerald-400/50" />}
            </div>
            ...
            {cashFlow ? (
              ...
              <div className="py-8 text-center">
                <button
                  ...
                  className="group flex flex-col items-center gap-3 mx-auto text-white/20 hover:text-emerald-400 transition-colors"
                >
                  {loadingCashFlow ? (
                    <Loader2 size={24} className="animate-spin text-emerald-400/50" />
                  ) : (
                    <LineChart size={24} className="group-hover:scale-110 transition-transform" />
                  )}
                  <span className="text-[9px] uppercase tracking-widest font-black">
                    {loadingCashFlow ? 'Calcul en cours...' : 'Prévision 30 Jours'}
                  </span>
                </button>
              </div>
            )}
