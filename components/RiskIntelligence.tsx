...
import { 
  ...
  Sparkles, Clock, Loader2, BrainCircuit, ShieldOff
} from 'lucide-react';
...
import { analyzePortfolioStrategically, detectFraud } from '../services/geminiService.ts';
...
const RiskIntelligence: React.FC<RiskIntelligenceProps> = ({ checks, currency, highValueThreshold, onViewCheck }) => {
  ...
  const runFraudDetection = async () => {
    ...
    const result = await detectFraud(checks);
    setFraudReport(result);
    ...
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-10">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button 
            ...
            className="bg-rose-500/10 border border-rose-500/30 text-rose-400 px-6 py-4 rounded-[16px] font-black text-[10px] uppercase tracking-widest hover:bg-rose-500/20 active:scale-95 transition-all flex items-center gap-2"
          >
            {isDetectingFraud ? <Loader2 size={14} className="animate-spin" /> : <ShieldOff size={14} />}
            Détection Fraude
          </button>
          <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-8 rounded-[22px] bg-rose-500/[0.03] border-rose-500/10 relative overflow-hidden">
            <h5 className="text-[10px] font-black text-rose-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
              <ShieldOff size={14} /> Rapport Fraude
            </h5>
            <div className="space-y-4">
              {fraudReport ? (
                <div className="text-[11px] text-white/70 leading-relaxed italic prose prose-invert max-w-none">
                ...
                <div className="py-10 text-center text-[10px] text-white/20 uppercase font-black italic">
                   Lancez la détection de fraude pour analyser les patterns suspects.
                </div>
              )}
            </div>
          </div>
