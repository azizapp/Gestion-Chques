...
import { classifyClients, generateMonthlyReport } from '../services/geminiService.ts';
...
const Reports: React.FC<ReportsProps> = ({ checks, currency }) => {
  ...
  const runClientClassification = async () => {
    setLoadingClients(true);
    const result = await classifyClients(checks);
  ...
  const runMonthlyReport = async () => {
    ...
    const result = await generateMonthlyReport(checks, currency);
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button 
            onClick={runClientClassification}
            disabled={loadingClients}
            className="p-3 bg-blue-500/10 border border-blue-500/30 text-blue-400 rounded-[12px] font-black text-[9px] uppercase tracking-widest hover:bg-blue-500/20 transition-all flex items-center gap-2 px-4"
          >
            {loadingClients ? <Loader2 size={14} className="animate-spin" /> : <Users size={14} />}
            Classer Clients
          </button>
          <button 
            onClick={runMonthlyReport}
            disabled={loadingMonthly}
            className="p-3 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-[12px] font-black text-[9px] uppercase tracking-widest hover:bg-emerald-500/20 transition-all flex items-center gap-2 px-4"
          >
            {loadingMonthly ? <Loader2 size={14} className="animate-spin" /> : <CalendarDays size={14} />}
            Rapport Mensuel
          </button>
        </div>
      </div>
      ...
      {/* AI Reports Section */}
      {(clientReport || monthlyReport) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {clientReport && (
            <div className="glass-card p-8 rounded-[14px] border-blue-500/10 bg-blue-500/[0.02]">
              <h4 className="text-lg font-bold flex items-center gap-3 mb-6 uppercase tracking-tighter">
                <Users className="text-blue-400" size={20} />
                Classification des Clients
              </h4>
              <div className="text-[11px] text-white/70 leading-relaxed italic whitespace-pre-line">
                {clientReport}
              </div>
            </div>
          )}
          {monthlyReport && (
            <div className="glass-card p-8 rounded-[14px] border-emerald-500/10 bg-emerald-500/[0.02]">
              <h4 className="text-lg font-bold flex items-center gap-3 mb-6 uppercase tracking-tighter">
