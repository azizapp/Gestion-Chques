...
import { Camera, Save, Globe, Building2, Bell, Shield, Coins, Smartphone, CalendarDays, Loader2, Building, BrainCircuit, Eye, EyeOff } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto space-y-10 pb-20 animate-in fade-in duration-700">
      ...
      <div className="glass-card p-10 rounded-[2.5rem] border-white/5 shadow-2xl space-y-8">
        <div className="flex items-center gap-4 text-gold">
          ...
          <h4 className="text-sm font-black tracking-[0.2em] uppercase">CONFIGURATION IA</h4>
        </div>
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              <input
                ...
                placeholder="AIzaSy..."
                className="w-full bg-[#0A0D18] border border-white/5 rounded-2xl py-4 px-6 pr-14 text-white text-sm font-bold outline-none focus:border-gold/30 transition-all font-mono tracking-wider"
              />
              <button
                type="button"
          ...
          <div className="flex items-center gap-4">
            <div className={`w-2.5 h-2.5 rounded-full ${localSettings.gemini_api_key ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.5)]' : 'bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.4)]'}`} />
            <span className={`text-[10px] font-black uppercase tracking-widest ${localSettings.gemini_api_key ? 'text-emerald-400/80' : 'text-amber-400/80'}`}>
              {localSettings.gemini_api_key ? 'CLÉ CONFIGURÉE' : 'NON CONFIGURÉE'}
            </span>
          </div>

          <p className="text-[10px] text-white/20 font-medium leading-relaxed">
            Obtenez votre clé API gratuite sur <span className="text-gold/60 font-bold">ai.google.dev</span> — La clé est chiffrée avant d'être stockée en base de données.
          </p>
        </div>
      </div>
    </div>
