...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Strip API key & logo_url from localStorage cache for security & quota
    const { gemini_api_key, logo_url, ...cacheSafe } = settings;
    localStorage.setItem('finansse_settings_cache', JSON.stringify(cacheSafe));
  }, [settings]);
  ...
  const syncWithServer = useCallback(async () => {
    ...
    try {
      ...
      if (settingsRes.data) {
        // Decode the encrypted API key from DB
        let decodedApiKey = '';
        if (settingsRes.data.gemini_api_key) {
          try { decodedApiKey = atob(settingsRes.data.gemini_api_key); } catch { decodedApiKey = ''; }
        }
        if (decodedApiKey) setGeminiApiKey(decodedApiKey);

        setSettings({
          ...DEFAULT_SETTINGS,
          ...settingsRes.data,
          gemini_api_key: decodedApiKey,
  ...
  const handleSaveSettings = async (newSettings: SystemSettings) => {
    ...
    if (newSettings.gemini_api_key) setGeminiApiKey(newSettings.gemini_api_key);
    if (isConfigured && session) {
      // Encode API key before saving to DB
      const dbPayload = {
        ...newSettings,
        gemini_api_key: newSettings.gemini_api_key ? btoa(newSettings.gemini_api_key) : null,
        user_id: session.user.id,
        updated_at: new Date().toISOString()
      };
      const { error } = await supabase.from('cheque_settings').upsert(dbPayload, { onConflict: 'user_id' });
      if (error) {
